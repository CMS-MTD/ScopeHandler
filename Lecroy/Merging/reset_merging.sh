echo "Turning off the flags."
echo -n "False" > /home/etl/Test_Stand/module_test_sw/merging.txt
echo -n "False" > /home/etl/Test_Stand/ETL_TestingDAQ/ScopeHandler/Lecroy/Conversion/merging.txt
echo -n "False" > /home/etl/Test_Stand/ETL_TestingDAQ/ScopeHandler/Lecroy/Acquisition/merging.txt

# echo "Turning on the flags."
#echo -n "True" > /home/etl/Test_Stand/ETL_TestingDAQ/module_test_sw/merging.txt
#echo -n "True" > /home/etl/Test_Stand/ETL_TestingDAQ/ScopeHandler/Lecroy/Conversion/merging.txt
#echo -n "True" > /home/etl/Test_Stand/ETL_TestingDAQ/ScopeHandler/Lecroy/Acquisition/merging.txt
